...
class TokenService {
    async generateAuthTokens(user: Pick<IUser, "_id">) {
        ...
        await new TokenModel({
            ...
            userId: user._id,
            expiresAt: Date.now() + CONFIGS.REFRESH_TOKEN_JWT_EXPIRES_IN,
        }).save();
        ...
        const refreshTokenJWT = JWT.sign({ _id: user._id, refreshToken: refreshToken }, CONFIGS.JWT_SECRET, {
            expiresIn: CONFIGS.REFRESH_TOKEN_JWT_EXPIRES_IN / 1000,
        });
    ...
    async refreshAuthTokens(refreshTokenJWT: string) {
        ...
        const decodedRefreshToken = JWT.verify(refreshTokenJWT, CONFIGS.JWT_SECRET, (err: any, decoded: any) => {
            if (err) throw new CustomError("Refresh token JWT expired", "-middleware/token-expired");
            return decoded;
        ...
        if (refreshTokens.length === 0) throw new CustomError("Invalid or expired token", "-invalid-expired-token");

        // Get user from database
        const user = await UserModel.findOne({ _id: decodedRefreshToken._id });
        if (!user) throw new CustomError("-invalid-expired-token");
        ...
        throw new CustomError("Invalid or expired token", "-invalid-expired-token");
    }
    ...
    async generateOtpToken({ userId, tokenType }: { userId: string; tokenType: Exclude<IToken["type"], "refresh-token"> }) {
        ...
        await new TokenModel({
            ...
            type: tokenType,
            userId: userId,
            expiresAt: Date.now() + CONFIGS.DEFAULT_DB_TOKEN_EXPIRY_DURATION,
        }).save();
